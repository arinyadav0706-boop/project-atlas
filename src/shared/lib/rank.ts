...
// 8 random base-62 chars ≈ 2.18e14 space — collisions among the handful of truly
...
// The suffix = a per-actor discriminator + random jitter (ADR-0010). The actor
// part means two *different* actors can never produce the same key even if the
// random parts ever coincided; the random part covers a single actor's own
// concurrent inserts. `actorId` is optional so the pure module stays usable
// without actor context (seed/backfill/tests). Only base-62 chars are kept so
// the suffix never contains the separator or a byte below it.
function suffix(actorId?: string): string {
  const actorPart = actorId
    ? actorId.replace(/[^0-9A-Za-z]/g, "").slice(-4)
    : "";
  let random = "";
  for (let i = 0; i < RANDOM_LEN; i++) {
    random += SUFFIX_ALPHABET[Math.floor(Math.random() * SUFFIX_ALPHABET.length)];
  }
  return actorPart + random;
}
...
export function rankBetween(
  ...
  if (fb !== null && fa !== null && fb === fa) {
    return fb + SEP + suffix(actorId);
  }
  return generateKeyBetween(fb, fa) + SEP + suffix(actorId);
}

// Append to the end of a column whose current last key is `last` (or empty).
export function rankAppend(last: string | null, actorId?: string): string {
  return rankBetween(last, null, actorId);
}
