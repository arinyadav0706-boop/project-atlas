...
describe("weeksOfWork (BR-5)", () => {
  it("treats a full reference week as 1", () => {
    expect(weeksOfWork(WEEKLY_CAPACITY_MINUTES)).toBe(1);
  });

  it("rounds to one decimal for display stability", () => {
    expect(weeksOfWork(360)).toBe(0.2); // 6h of a 40h week = 0.15 -> 0.2
  });

  it("is 0 for no remaining work", () => {
    expect(weeksOfWork(0)).toBe(0);
  });
});
