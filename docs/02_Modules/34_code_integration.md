...
| BR-12 | ~~**No outbound calls to the provider.**~~ **Superseded by module 35 / ADR-0054.** Reads now happen, authenticated by a provider app install. Webhook-driven linking — everything else in this document — still makes no outbound call, and posting *back* to the provider is still out of scope. |
