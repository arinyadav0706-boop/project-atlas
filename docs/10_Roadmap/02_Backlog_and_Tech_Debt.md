...
| DB-8 | Reorder concurrency hardening | P3 | No | PARTIAL | **Collision-free keys DONE (ADR-0010):** every rank carries a random suffix so concurrent same-gap drops can't produce equal keys; unique index `(projectId, status, rank)` is the loud backstop; covered by unit (1000 same-gap inserts) + integration tests. **Still open:** (a) optimistic-concurrency guard for *same-card* moves (today last-writer-wins — a lost-update, distinct from the collision this closed); (b) on-demand re-rank **repair tool** to tidy any fractional-key clusters left by a prior collision. |
