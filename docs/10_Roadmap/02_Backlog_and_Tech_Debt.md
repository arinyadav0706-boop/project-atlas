...
| FUT-16 | Labels: **creation-lockdown toggle + merge + usage counts** | P3 | No | OPEN | ADR-0018 Phase 2. RBAC is already a policy function (`canCreateLabel`), so lockdown is a config flip; merge/usage-counts need new endpoints. Build when label entropy actually appears. |
