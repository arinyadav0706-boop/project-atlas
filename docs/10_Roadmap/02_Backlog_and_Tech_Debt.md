...
| GL-3 | **Rate limiting** on auth + mutation endpoints | P1 | PARTIAL (auth + search done) | **DB-backed fixed-window limiter shipped (ADR-0028, security finding F1):** portable (plain Postgres, no new infra), atomic under concurrency (`rate_limits` table, `INSERT … ON CONFLICT … RETURNING`). **Applied:** credential login (per IP+email, 8/15min — fails like an invalid credential, no account-existence signal) and search (per user, 60/min). Reusable `enforceRateLimit()` + `RateLimitError`→429+`Retry-After` for the rest. **Follow-up:** roll `enforceRateLimit(mutation)` across all write Route Handlers (tracked). |
