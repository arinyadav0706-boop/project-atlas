...
| FUT-8 | Sprint: **reorder the planned-sprint queue** (drag to reorder sprints themselves) | P3 | No | OPEN | Sprints list ACTIVE-first then PLANNED by `createdAt` (ADR-0015). Reordering the queue needs a sprint-level order key (a `rank`-style column on `Sprint`); issues *within* a sprint already reorder. |
