...
| DB-8 | Reorder concurrency hardening (unique rank per column + regenerate-on-conflict) | P3 | No | OPEN | Today two simultaneous drops into the *same* gap can produce equal ranks — no crash/loss, resolved by the `id` tiebreaker, self-heals on next drag. For real multi-user load: add a unique partial index on `(projectId, status, rank) WHERE deletedAt IS NULL` and, on the rare unique-violation, catch + regenerate `generateKeyBetween` against the updated neighbours (small retry in `IssueService.reorder`). Pair with a concurrency integration test. |
