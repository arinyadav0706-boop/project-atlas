...
| DB-8 | Reorder concurrency hardening | P3 | No | PARTIAL | **Collision-free keys DONE (ADR-0010).** **Same-card lost-update DONE for reorder (ADR-0011):** `version` column + conditional write; a stale drag gets a `409` instead of silently overwriting; unit + integration tests. **Still open:** (a) extend the enforced `version` check to **edit/transition/delete** (the higher-value "lost edited text" case — column + increment already in place, just add the `WHERE version` guard); (b) on-demand re-rank **repair tool** for any fractional-key clusters from a prior collision. |
