...
<<<<<<< Updated upstream
| UI-1 | **Shared chart kit + rebuild the velocity chart on it** | P2 | No | OPEN | `docs/05_UI/03_Data_Visualisation.md` §2 audits the current velocity bars: no axis, no gridlines, bars scaled to the tallest bar (so every chart looks "full" and the shape carries no information), no average line, truncated sprint labels, issue counts reachable only via a `title` tooltip (invisible on touch), a 2px sliver for a genuine zero. Build `src/shared/components/charts/` (ChartFrame, Bars, Line, Donut, HeatGrid, Sparkline) as plain themed SVG — no chart library (§3) — and rebuild velocity on it. Blocks every future report looking consistent. |
| UI-2 | Workload: team distribution bar + person bar chart with a real axis | P3 | No | OPEN | Charts before reading (`03_Data_Visualisation.md` §7). One stacked bar for the team's status mix, then person rows as a proper bar chart with ticks at 1 and 2 weeks. No schema change. |
| UI-3 | Status-breakdown donut uses hard-coded hex colours | P4 | No | OPEN | `report-card.tsx` `STATUS_COLORS` ignores the theme and would break a white-label deployment. Move to theme tokens (§6). |
=======
| UI-1 | **Shared chart kit + rebuild the velocity chart on it** | P2 | No | ✅ DONE 2026-08-06 | `docs/05_UI/03_Data_Visualisation.md` §2 audits the current velocity bars: no axis, no gridlines, bars scaled to the tallest bar (so every chart looks "full" and the shape carries no information), no average line, truncated sprint labels, issue counts reachable only via a `title` tooltip (invisible on touch), a 2px sliver for a genuine zero. Build `src/shared/components/charts/` (ChartFrame, Bars, Line, Donut, HeatGrid, Sparkline) as plain themed SVG — no chart library (§3) — and rebuild velocity on it. Blocks every future report looking consistent. |
| UI-2 | Workload: team distribution bar + person bar chart with a real axis | P3 | No | OPEN (now via ECharts, ADR-0036) | Charts before reading (`03_Data_Visualisation.md` §7). One stacked bar for the team's status mix, then person rows as a proper bar chart with ticks at 1 and 2 weeks. No schema change. |
| UI-3 | Status-breakdown donut uses hard-coded hex colours | P4 | No | ✅ DONE 2026-08-06 | `report-card.tsx` `STATUS_COLORS` ignores the theme and would break a white-label deployment. Move to theme tokens (§6). |
>>>>>>> Stashed changes
| WL-3 | **Time-phased workload (people × weeks grid)** | P3 | No | PROPOSED | ADR-0035. The biggest gap vs ClickUp/Asana: today's view says "2 weeks queued", not "slammed next week, free the week after". No schema change in Phase 1 — issues bucket by `dueDate`, else sprint `endDate`, else an explicit **Unscheduled** column. Four open questions in the ADR to settle first. |
| WL-4 | `Issue.startDate` so effort spreads across weeks instead of spiking in the due week | P4 | No | OPEN | ADR-0035 Phase 2. Without it a 3-week task lands entirely in its due week. Needs a doc + migration + UI; likely decided together with the waterfall/Gantt module, which wants the same field. |
