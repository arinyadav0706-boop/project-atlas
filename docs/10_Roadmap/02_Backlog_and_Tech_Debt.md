...
| FUT-6 | Sprint: **multi-sprint planning** (several `PLANNED` sprints at once) | P2 | No | OPEN | The hardening pass added edit/delete/history/dates/overdue (BR-8/9/10) but still shows ONE current sprint (ACTIVE, else next PLANNED). Jira-parity's remaining big gap: show/plan multiple PLANNED sprints at once (sprint picker or stacked sections). Backlog↔Sprint drag + one-active rule already generalise. Its own effort — flagged to the founder. |
