...
| FUT-14 | Labels/Components: **filter controls in Board/Backlog bars** | P2 | No | OPEN | The board filter *query* already supports `labelIds` + `componentIds` (ADR-0008 composable filter); only the multiselect UI controls are deferred. Needs the label/component catalogs threaded into the filter bar. |
| FUT-15 | Labels/Components: **chips on board cards / list rows / backlog rows** | P3 | No | OPEN | Requires `labels[]`/`components[]` on `IssueListItemDto` — the 5-mapper cross-cutting change (see TD-1). Issue detail already shows chips. |
